...
class HRDAHead(BaseDecodeHead):
    ...
    def decode_hr(self, inp, bs):
        if isinstance(inp, dict) and 'boxes' in inp.keys():
            ...
            # mmcv.print_log(f'2:', 'mmseg')
            crop_seg_logits = self.head(features)[0]
            for i in range(len(boxes)):
                y1, y2, x1, x2 = boxes[i]
            ...
            # mmcv.print_log(f'2222:', 'mmseg')
            return self.head(inp)[0]

    def get_scale_attention(self, inp):
        if self.scale_attention is not None:
            # mmcv.print_log(f'3333:', 'mmseg')
            att = torch.sigmoid(self.scale_attention(inp)[0])
        else:
    ...
    def forward(self, inputs):
        assert len(inputs) == 2
        # for input0 in inputs[0]:
        #     mmcv.print_log(f'input0: {type(input0)}', 'mmseg')
        # for input1 in inputs[1]:
        #     mmcv.print_log(f'input1: {type(input1)}', 'mmseg')
        ...
        # mmcv.print_log(f'1111:', 'mmseg')
        lr_seg = self.head(lr_inp)[0]
        # mmcv.print_log(f'lr_seg: {lr_seg.shape}', 'mmseg')
        # print_log(f'lr_seg {lr_seg.shape}', 'mmseg')

        hr_seg = self.decode_hr(hr_inp, batch_size)

        # mmcv.print_log(f'lr_sc_att_inp.shape: {lr_sc_att_inp.shape}', 'mmseg')
        ...
        # mmcv.print_log(f'fused_seg.size(): {fused_seg.size()}', 'mmseg')
        # mmcv.print_log(f'lr_seg.size(): {lr_seg.size()}', 'mmseg')
        # mmcv.print_log(f'hr_seg.size(): {hr_seg.size()}', 'mmseg')
    ...
    def forward_train(self,
        ...
        #     mmcv.print_log(f'logits: {logits.shape}', 'mmseg')
        # for gt_semantic in gt_semantic_seg:
        #     mmcv.print_log(f'gt_semantic: {gt_semantic.shape}', 'mmseg')    
    ...
    def get_lr_rep(self, inputs):
        assert len(inputs) == 2
        lr_inp = inputs[0]
        x = self.head(lr_inp)[1]
        # rep = self.head.getRep()
        # mmcv.print_log(f'rep1.shape: {rep.shape}', 'mmseg')
        # mmcv.print_log(f'rep1: {rep}', 'mmseg')
        return x
