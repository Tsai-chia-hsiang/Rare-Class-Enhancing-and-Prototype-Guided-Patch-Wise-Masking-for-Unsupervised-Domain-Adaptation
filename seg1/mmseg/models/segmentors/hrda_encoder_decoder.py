...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    # def generate_small_pseudo_label(self, img, img_metas):
    #     self.update_debug_state()
    #     out = self.small_encode_decode(img, img_metas)
    #     if self.debug:
    #         self.debug_output = self.decode_head.debug_output
    #     return out
    
    # def small_encode_decode(self, img, img_metas, upscale_pred=True):
    ...
    #                 scaled_img.detach()
    #     # for mres_feats1 in mres_feats[0]:
    #     #     mmcv.print_log(f'mres_feats1: {mres_feats1.shape}', 'mmseg')
    #     # for mres_feats2 in mres_feats[1]:    
    #     #     mmcv.print_log(f'mres_feats2: {mres_feats2}', 'mmseg')        
    #     small_out = self._decode_head_forward_test(mres_feats, img_metas)
    #     # mmcv.print_log(f'small_out.size(): {small_out.size()}', 'mmseg')  
       
    #     return small_out
    
    def getRep(self, img, img_metas, upscale_pred=True):
        mres_feats = []
        self.decode_head.debug_output = {}
        for i, s in enumerate(self.scales):
            if s == 1 and self.blur_hr_crop:
            ...
            if i >= 1 and self.hr_slide_inference:
                ...
                mres_feats.append(self.extract_unscaled_feat(scaled_img))
        rep = self._decode_head_get_rep(mres_feats)
        del mres_feats
        # with torch.no_grad():
        #     rep = resize(
        #         input=rep,
        ...
        #         mode='bilinear',
        #         align_corners=self.align_corners)
        # mmcv.print_log(f'self.align_corners: {self.align_corners}', 'mmseg')
    ...
    def encode_decode(self, img, img_metas, upscale_pred=True):
        ...
        for i, s in enumerate(self.scales):
            ...
            if self.decode_head.debug:
                if s ==0.5:
                    self.decode_head.debug_output[f'Image'] = \
                        scaled_img.detach()
                # self.decode_head.debug_output[f'Img {i} Scale {s}'] = \
                #     scaled_img.detach()           
        # for mres_feats1 in mres_feats[0]:
    ...
    def get_small_out(self):
        
        # mmcv.print_log(f'self.small_out.size(): {self.small_out.size()}', 'mmseg')  
        return self.small_out
    def get_tgt_rep(self):
        rep = self._decode_head_get_rep(self.mres_feats)
        # mmcv.print_log(f'self.small_out.size(): {self.small_out.size()}', 'mmseg')  
        return rep
